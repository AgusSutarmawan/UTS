<x-template-layout>
        <h1 class="font-semibold text-xl bg-black text-white" style="text-align: center">
            <img src="realmadrid.png " width="50px" class="mx-auto d-block">{{$title}}
        </h1>
        <div class="shadow px-6 py-4 bg-black text-white rounded sm:px-1 sm:py-1 sm:bg-gray-100">
            <div class="grid grid-cols-12">
                <div class="col-span-6 p-4 bg-white">
                    <a href="{{route('player.create')}}"><button class="px-4 py-1 text-sm rounded text-green-600 font-semibold border border-green-200 hover:text-white hover:bg-green-600 hover:border-transparent focus:outline-none">Add</button></a>
                </div>
                <form class="col-span-6 p-4 flex justify-end bg-white" method="GET" action="/player">
                    ...
                    <tbody class="bg-white text-black divide-y divide-gray-200">
                        ...
                        @foreach ($player as $item )
                            <tr>
                                ...
                                <td>{{$item->position}}</td>
                                <td>{{$item->age}}</td>
                                <td><a href="{{route('player.edit',$item->id)}}"><button class="px-4 py-1 text-sm rounded text-blue-600 font-semibold border border-blue-200 hover:text-white hover:bg-blue-600 hover:border-transparent focus:outline-none">Edit</button></a></a>
